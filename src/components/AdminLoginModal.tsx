...
import { X, Shield, KeyRound, ArrowRight, AlertCircle, Sparkles } from 'lucide-react';
...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Default admin secret is admin123 or whatever key user enters (>= 4 chars)
    if (cleanSecret === 'admin123' || cleanSecret.length >= 4) {
      try {
        safeSessionStorage.setItem('monitor_admin_token', 'valid');
      } catch (err) {
        console.warn('Could not write admin token:', err);
      }
      onLoginSuccess();
      onClose();
    } else {
      setError('密钥无效，默认管理员密钥为 admin123');
    }
  };
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 dark:bg-cyan-500 dark:hover:bg-cyan-400 text-white dark:text-slate-950 font-bold text-xs transition-colors shadow-xs cursor-pointer"
            >
              <span>验证并登录</span>
